...
-- Map leader + 4 to the function
vim.keymap.set('n', '<leader>4', turn_off_background, { noremap = true, silent = true, desc = 'Turn off background color' })
...
-- 07/11 Disabled netrw for NeoTree
    ...
    config = function()
      ...
      local servers = {
        clangd = {
          ...
          filetypes = { 'c', 'cpp', 'objc', 'objcpp', 'h', 'hpp' }, -- File types to activate clangd
          root_dir = require('lspconfig.util').root_pattern('.git', 'compile_commands.json', 'compile_flags.txt', 'Makefile'), -- Root directory to determine the project root
          settings = {
            clangd = {
              -- Example: enable the clang-tidy integration
              clang_tidy = true,
              -- Example: specify additional arguments for clangd
              args = { '--completion-style=detailed' },
            },
          },
          -- Example: customize the capabilities for clangd
...
local wsl_config = vim.fn.expand '~/.config/nvim/lua/custom/plugins/wsl.lua'
if vim.fn.filereadable(wsl_config) == 1 then
  dofile(wsl_config)
end
